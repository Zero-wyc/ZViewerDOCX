...
- 使用域名并通过 Let's Encrypt 申请可信证书，详见 [HTTPS 与证书](/basic/https)。
...
- 大会员专享内容需在后台配置有效的 Bilibili 登录凭证，或使用 [ZViewerCLI 本地代理](/basic/cli)。
...
系统支持从 GitHub Releases 自动检测并应用更新，也支持手动上传压缩包更新。管理员可在管理后台控制是否接收预发布版（main 分支自动构建）的更新，详见[部署方式](/advanced/build-update)。
